...
import {
  default as TDSTextFieldBig,
  Props as TDSTextFieldBigProps,
} from '../@tossteam/tds-web/react/components/TextFieldBig/TextFieldBig';
...
const propsByTypeControlLabel: {
  [key in TypeLabel]: TDSTextFieldBigProps;
} = {
  '📄 일반 텍스트': {},
  '😀 이름': { type: 'name' },
  '💰 금액': { type: 'tel' },
  '🚧👤 주민번호 앞 6자리': { type: 'tel' },
  '🚧👤 주민번호 뒤 7자리': { type: 'tel', textSecurity: true },
  '📱 전화번호': { type: 'tel' },
  '🔐 비밀번호': { type: 'tel', textSecurity: true },
  '0️⃣ 숫자': { type: 'tel' },
};

const autoFocusByControlLabel: {
  [key in AutoFocusLabel]: TDSTextFieldBigProps['autoFocus'];
} = {
  ON: true,
  OFF: false,
};
...
export function TextFieldBig(props: Props) {
  const {
    type,
    inputValue,
    helpText,
  ...
  return (
    <TDSForm>
      <TDSForm.Group label={rest[`${defaultValueByType[type].key}Label`]} help={helpText} hasError={state === '에러'}>
        <TDSComponent
          name="TextFieldBig"
          summary={{
...
addPropertyControls(TextFieldBig, {
  type: {
    title: '종류',
...
(TextFieldBig as any).defaultProps = {
  width: 375,
  height: 74,
};

TextFieldBig.displayName = makeComponentDisplayName('TextFieldBig', '텍스트필드빅');
